...
FriendlyEats.prototype.initBookingDialog = function() {
  var dialog = document.querySelector('#dialog-booking');
  this.dialogs.booking = new mdc.dialog.MDCDialog(dialog);

  var that = this;
  this.dialogs.booking.listen('MDCDialog:accept', function() {
    var pathname = that.getCleanPath(document.location.pathname);
    var id = pathname.split('/')[2];

    that.addBooking(id, {
      userName: dialog.querySelector('#name').value, 
...
FriendlyEats.prototype.viewRestaurant = function(id) {
  ...
  return this.getRestaurant(id)
    .then(function(doc) {
      ...
      var dialogBooking = that.dialogs.booking;
      
      data.show_booking = function() {
        dialogBooking.show();
      };
