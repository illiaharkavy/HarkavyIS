...
FriendlyEats.prototype.addBooking = function (restaurantID, booking) {
  const collection = firebase.firestore().collection('restaurants');
  const document = collection.doc(restaurantID);
  const newRatingDocument = document.collection('booking').doc();

  return firebase.firestore().runTransaction((transaction) => {
    return transaction.get(document).then((doc) => {

      return transaction.set(newRatingDocument, booking);
    });
  });
